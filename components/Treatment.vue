<template>
    ...
    <!-- The Modal -->
        ...
        <div class="w-100 text-right py-5 px-0">
            <button @click="create_treatment = !create_treatment, getTreatments()" 
            :class="create_treatment ? 'btn-primary shadow px-5' : 'text-primary'"
            class="btn btn-large btn-flat waves-effect waves-light transition transparent font-weight-bold">
                <span class="d-flex align-items-center">
                    ...
                    <span class="transition">Add Treatment</span>
                </span>
            </button>
        </div>
        <create-treatment @record_added="create_treatment = false, getTreatments()" v-if="create_treatment" class="animate__animated animate__fadeInUp animate__faster" />
        <div v-else>
                ...
                <table id="record-table" class="table table-responsive-md w-100 striped animate__animated animate__zoomIn animate__faster" style="overflow: auto;">
                    ...
                    <tbody>
                        <tr v-for="(treatment, i) in treatments" :key="i">
                            ...
                            <td class="py-4">{{ treatment.title }}</td>                        
                            <td class="py-4">{{ treatment.symptoms }}</td>                        
                            <td class="py-4 text-center">
                                <button @click="c = treatment" data-toggle="modal" type="button" data-target="#details-modal"  class="btn btn-primary waves-effect waves-light px-4">View</button>
                            </td>
                            <td class="text-right">
                                <button @click="remove(treatment, i)" class="btn mdi mdi-delete btn-flat waves-light waves-effect transparent"></button>
                            </td>  
                        </tr>
                    </tbody>
                </table>           
...
export default {
  components: { CreateTreatment },
    ...
    methods: {
        remove(record, i) {
            ...
            this.firebase_remove_one(ref, (res) => {
                M.toast({ html: "Treatment Deleted Successfully" })
        ...
        getTreatments() {
            ...
            this.firebase_get("treatments", (treatments) => {
                this.loading = false
                this.treatments = treatments.reverse()
    ...
    mounted() {
        ...
        M.toast({ html: "Fetching treatments..." })
